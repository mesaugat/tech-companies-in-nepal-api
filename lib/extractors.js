...
const { isNil } = lang;
const { splitAndTrim, logger } = utils;
...
const extractKeywords = description => {
  ...
  return promise.then(({ data }) => {
    return data.keywords.map(kw => {
      ...
      let adjusted = keyword.toLowerCase().replace(/[^\w\s]+/g, '');
...
const extractLocations = location => {
  ...
  logger.info(`Parsed locations: ${locations}`);
