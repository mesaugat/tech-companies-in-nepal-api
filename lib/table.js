 ...
 *
 * Direct copy of: https://github.com/poteto/hww-api/blob/6361831a2d1fa1f37127fb21fe1c7ca4093e7eae/lib/table.js
 *
 * @class Table
 */
class Table {
  ...
  where(query) {
    return new Promise((resolve, reject) => {
      this.client.select({ filterByFormula: query }).eachPage(resolve, err => {
        if (err) {
          reject(err);
        }
      });
  ...
  create(props) {
    return new Promise((resolve, reject) => {
      this.client.create(props, (err, record) => {
        if (!err) {
          resolve(record);
        } else {
          reject(err);
  ...
  findOrCreateBy(query, props) {
    return this.where(query).then(records => {
      if (records.length) {
        return head(records);
      }
  ...
  update(id, props, options) {
    return new Promise((resolve, reject) => {
      this.client.update(id, props, options, (err, record) => {
        if (!err) {
          resolve(record);
        } else {
          reject(err);
