  ...
  return extractLocations(location).then(locationRecords => {
    ...
    return companiesTable
      ...
      .then(record => {
        ...
        return extractKeywords(description).then(keywords =>
          companiesTable.update(
            id,
            {
              Website: url,
      ...
      .then(record => logger.info(`OK: ${record.getId()} | ${record.get('Name')}`))
...
const processDiff = (rawDiff = '') => {
  ...
  if (isNil(extracted)) {
    logger.warn('Could not extract from diff', rawDiff);
